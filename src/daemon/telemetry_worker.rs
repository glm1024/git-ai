...
enum MetricsPersistRequest {
    ...
    /// Persist a control-plane submission and report the SQLite transaction
    /// outcome before the caller acknowledges it.
    StoreAwait {
...
impl DaemonTelemetryWorkerHandle {
    ...
    pub async fn submit_telemetry(
        ...
        self.persist_metrics_and_wait(metric_events).await
    }

    /// Submit daemon-owned telemetry without back-pressuring its core runtime.
    /// Metric loss on a full bounded queue is counted by
    /// `enqueue_metrics_persist`; external control-plane submissions use the
    /// durable acknowledgement path above instead.
    async fn submit_telemetry_best_effort(&self, envelopes: Vec<TelemetryEnvelope>) {
        let (buffered_envelopes, metric_events) = split_metric_envelopes(envelopes);
        if !buffered_envelopes.is_empty() {
            self.buffer
                .lock()
    ...
    async fn persist_metrics_and_wait(
        &self,
        ...
        self.metrics_persist_tx
            .send(MetricsPersistRequest::StoreAwait {
                events: metric_events,
                completion: completion_tx,
            })
            .await
            .map_err(|_| {
                GitAiError::Generic("metrics persistence worker has stopped".to_string())
            })?;
        completion_rx
            .await
            .map_err(|_| {
                GitAiError::Generic(
                    "metrics persistence worker failed before acknowledgement".to_string(),
                )
            })?
...
fn submit_daemon_internal_telemetry_with_handle(
    ...
    if let Ok(runtime) = tokio::runtime::Handle::try_current() {
        runtime.spawn(async move {
            handle.submit_telemetry_best_effort(envelopes).await;
        });
    } else {
...
pub fn spawn_telemetry_worker() -> DaemonTelemetryWorkerHandle {
    ...
    spawn_token_bucket_repair(telemetry_runtime);
...
async fn metrics_persist_loop(mut rx: tokio::sync::mpsc::Receiver<MetricsPersistRequest>) {
    while let Some(request) = rx.recv().await {
        match request {
            ...
            MetricsPersistRequest::StoreAwait { events, completion } => {
                let result = tokio::task::spawn_blocking(move || store_metrics_in_db(&events))
                    .await
                    .map_err(|e| format!("metrics persistence task panicked: {e}"))
                    .and_then(|result| result.map(|_| ()).map_err(|e| e.to_string()));
...
fn spawn_token_bucket_repair(telemetry_runtime: &tokio::runtime::Runtime) {
...
fn should_deliver_metric_event(config: &Config, event: &MetricEvent) -> bool {
    ...
    if requires_remote
        && repo_url
            .as_deref()
            .is_none_or(|url| crate::repo_url::normalize_repo_url(url).is_err())
    {
        return false;
    }
